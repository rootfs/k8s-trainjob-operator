...
Agents coordinate through `agents/AGENCY.md`, a git-tracked file that acts as a task board with ownership, status, dependencies, and notes.
...
│  Component Table:                                           │
│  ┌──────────────────────┬──────────┬────────────┬────────┐ │
│  │ Component            │ Owner    │ Status     │ Branch │ │
│  ├──────────────────────┼──────────┼────────────┼────────┤ │
│  │ initial-model-regist │ model-b  │ in_progress│ agent/ │ │
│  │ prolog-ib-detection  │ null     │ pending    │ -      │ │
│  │ install-conversion   │ model-i  │ proposed   │ agent/ │ │
│  └──────────────────────┴──────────┴────────────┴────────┘ │
│                                                             │
│  Dependency Graph:                                          │
│  initial-model-registry ──► auto-config-test-coverage       │
│  eval-job-builder ──► eval-phase-reconciler                 │
│       ──► install-conversion-job                            │
│            ──► install-registry-push                        │
│                 ──► install-serving-config                   │
│                                                             │
│  Agent Notes:                                               │
│  (agents append summaries of completed work here)           │
└─────────────────────────────────────────────────────────────┘
...
agents/
  ...
  roles/
    model-builder/SKILL.md       Model architecture and registry management instructions
    model-trainer/SKILL.md       Auto-parallelism and training config instructions
    infra/SKILL.md               CRD, resource builders, Kueue integration instructions
    ops/SKILL.md                 Reconciler and webhook logic instructions
    sre/SKILL.md                 GPU health checks and monitoring instructions
  ...
  manifests/
    ...
    agent-cronjob.yaml           Periodic agent CronJobs (SRE, CI/CD)
    rbac.yaml                    Namespace, ServiceAccount, Role, RoleBinding
    git-secret.yaml              Git credentials Secret template

Makefile                         Build, test, lint, docker targets (operator + agents)
